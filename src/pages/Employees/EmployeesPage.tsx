    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col sm:flex-row justify-between sm:items-center gap-4'>
        <div>
          ...
          <p className='text-sm text-slate-500 mt-0.5'>
            Tổng số: <span className='font-semibold text-slate-700'>{mockUsers.length}</span> nhân viên trong hệ thống
          </p>
        </div>
      ...
      <div className='bg-white p-4 rounded-xl border border-slate-200 shadow-xs'>
        ...
        <div className='flex flex-wrap items-center justify-between gap-3 mt-3 pt-3 border-t border-slate-100 text-sm'>
          ...
          <span className='text-xs text-slate-400'>
            Bộ lọc tương thích với endpoint GET /api/users
          </span>
        </div>
      </div>
                  ...
                  <TableCell>
                    <div className='flex items-center gap-1.5'>
                      <Button
                        size='sm'
                        variant='secondary'
                        onPress={() => navigate(`/employees/${user.id}`)}>
                        Chi tiết
                      </Button>
                      <Button
                        size='sm'
      ...
      <div className='mt-6 bg-white p-3 rounded-xl border border-slate-200'>
        <Pagination className='w-full'>
          <Pagination.Summary>
            Hiển thị 1–{LIMIT} của {mockUsers.length} nhân viên
          </Pagination.Summary>
          <Pagination.Content>
            <Pagination.Item>
        ...
        <ModalBackdrop>
          <ModalContainer>
            <ModalDialog className='max-w-2xl'>
              <ModalHeader>
                <ModalHeading className='text-lg font-bold text-slate-900'>
                  Thêm Nhân Viên Mới
                </ModalHeading>
